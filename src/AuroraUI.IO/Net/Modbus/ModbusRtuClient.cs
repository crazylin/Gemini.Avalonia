...
{
    private static readonly ILogger Logger = LogManager.GetLogger("AuroraUI.IO.ModbusRtuClient");
    ...
    public void Start(string name, int baudRate)
    {
        ...
        {
            Logger.Error($"ModbusRtuClient start open port {name} failed: {e}");
            return;
        }
            ...
            while (_isRunning)
            {
                try
                {
                    ...
                    else
                    {
                        ...
                        var readCount = 0;
                        while (_isRunning && readCount < dataLen) 
                            tempBuf[readCount++] = (byte)_serialPort.ReadByte();
                ...
                {
                    Logger.Error($"ModbusRtuClient read data failed: {e}");
                    ++exceptionCount;
                    if (exceptionCount > 10)
                    {
                        Logger.Error("ModbusRtuClient read data failed too many times, thread exit.");
                        break;
                    }
        ...
        _writeThread = new Thread(() =>
        {
            while (_isRunning)
            {
                ...
                while (_writeQueue.TryDequeue(out var ds))
                {
                    ...
                    {
                        Logger.Error($"ModbusRtuClient write data failed: {e}");
                    }
    ...
    public void Stop()
    {
        ...
        {
            Logger.Error($"ModbusRtuClient stop failed: {e}");
        }
    }
}
