...
public class PipelineTcpClient<T> : IDisposable where T : class, IPipelineDataPackage<T>, new()
{
    ...
    /// 构造函数（使用TcpClient）
    /// </summary>
    /// <param name="tcpClient">TcpClient实例</param>
    ...
    public void Start()
    {
        ...
        _ = Task.Run(async () =>
        {
            try
            {
                while (_running)
                {
                    ...
                    {
                        Logger.Error($"PipelineTcpClient socket connection reset: {e}");
                        break;
                    }
                    catch (Exception e)
                    {
                        Logger.Error($"PipelineTcpClient socket receive failed: {e}");
                        break;
                    }
            ...
            {
                Logger.Error($"PipelineTcpClient pipe writing failed: {e}");
            }
            finally
            {
                // 通知管道写入端完毕
        ...
        _ = Task.Run(async () =>
        {
            while (_running)
            {
                ...
                {
                    Logger.Error($"PipelineTcpClient parse packet failed: {e}");
                    break;
                }
            ...
            {
                Logger.Error($"PipelineTcpClient pipe reader complete failed: {e}");
            }
            ...
            {
                Logger.Error($"PipelineTcpClient socket close failed: {e}");
            }
    ...
    public async Task<bool> SendAsync(T packet)
    {
        ...
        if (packet is IPacketWritable w)
        {
            ...
            try
            {
                ...
                if (sent != data.Length)
                {
                    Logger.Error($"发送数据不完整: 期望{sent}字节，实际发送{data.Length}字节");
                    return false;
                }
                return true;
            }
            ...
            {
                Logger.Error($"Socket发送异常: {ex.SocketErrorCode} - {ex.Message}");
                return false;
            }
            catch (Exception e)
            {
                Logger.Error($"PipelineTcpClient send message failed: {e}");
                return false;
            }
        }
    ...
    public void Close()
    {
        ...
        {
            Logger.Error($"PipelineTcpClient socket shutdown failed: {e}");
        }
        ...
        {
            Logger.Error($"PipelineTcpClient close failed: {e}");
        }
    }
    ...
    public void Dispose()
    {
        if (!_disposed)
        {
            ...
            {
                Logger.Error($"PipelineTcpClient dispose failed: {ex}");
            }
        }
    }
}
